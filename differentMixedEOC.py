...
class DifferentMixedEOC:
    def __init__(
        ...
        self.i_max = i_max
        self.degree = degree
        self.table_of_errors = np.zeros((i_max, 2), dtype=np.float64)
        self.table_of_error_rates = np.zeros((i_max - 1, 1), dtype=np.float64)
    ...
    def computeError(
        ...
        for i in range(self.i_max):
            ...
            # solver = PoissonSolverMixed.fromNSquare(n, self.degree,self.element1Family,self.element2Family,u_exact=u_exact)
                solver1 = PoissonSolverMixed.fromNRectangle(
                n,
                self.degree,
                self.element1Family,
                self.element2Family,
                ...
                solver2 = PoissonSolver.fromNRectangle(2**9, u_bc=pgn, degree=self.degree)

            elif self.domain_flag == 1:
                solver1 = PoissonSolverMixed.fromLshapeGmsh(
                n,
                self.degree,
                self.element1Family,
                self.element2Family,
                ...
                pgn = projection_pgn.solve()
                solver2 = PoissonSolver.fromLshapeGmsh(2**9, u_bc=pgn, degree=self.degree)

            sigmah1, _,_ = solver1.solve()
            uh2 = solver2.solve()
            # comm = uh.function_space.mesh.comm
            self.table_of_errors[i, 0] = 1 / n
            self.table_of_errors[i, 1] = self.__error(sigmah1, uh2)
    ...
    def __error(self, sigma1, uh2, degree_raise=3):
        domain = uh2.function_space.mesh
        elements = element(
            "RT",
            CellType.triangle,
        ...
        error_l2 = fem.form(inner(u_W - grad(uh2), u_W - grad(uh2)) * dx)
